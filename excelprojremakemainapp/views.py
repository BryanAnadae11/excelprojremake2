...
def hiddenIndex(request):
	return render(request, 'excelprojremakemainapp/newindex.html')
...
def about(request):
	return render(request, 'excelprojremakemainapp/newAbout.html')

def renewableEnergy(request):
	return render(request, 'excelprojremakemainapp/newRenewableEnergy.html')

def financialService(request):
	return render(request, 'excelprojremakemainapp/newFinancialService.html')

def energy(request):
	return render(request, 'excelprojremakemainapp/newEnergy.html')

def portfolioManagement(request):
	return render(request, 'excelprojremakemainapp/newPortfolioManagement.html')

def financialAdvising(request):
	return render(request, 'excelprojremakemainapp/newFinancialAdvising.html')
...
def wiretransfer(request):
	client= request.user.client
	client_name= request.user.username
	if request.method == 'POST':
		investment_plan_name= request.POST.get('investment_plan')
		amount= request.POST.get('amount')
		if investment_plan_name and amount:
			try:
				Wire_transfer_request.objects.create(
					client= client,
					investment_plan_name= investment_plan_name,
					amount= amount
					)
				send_mail(client_name, "A client with username: {} has created a wire transfer deposit request for an amount ${} and investment plan {}".format(client_name, amount, investment_plan_name),settings.EMAIL_HOST_USER, ['support@alphaoilhedge.com'])
				return HttpResponse('Request submitted successfully!')
			except:
				return HttpResponse('Unable to complete request. Please try again later!')
		else:
			return HttpResponse('Complete the form and try again')

	return render(request, 'excelprojremakemainapp/wiretransfer.html')


@login_required(login_url='signin')
def cashmailing(request):
	address= Cash_mailing_address.objects.all()
	last_address= address.last()
	context={'last_address':last_address}
	return render(request, 'excelprojremakemainapp/cashmailing.html', context)
...
def signup(request):
	...
	if form.is_valid():
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
...
def dashboard(request):
	if request.user.is_staff:
		...
		client_target_profit= client.Target_profit
	...
	'client_withdrawal':client_withdrawal, 'client_target_profit':client_target_profit, 'client_code':client_code, 'number_of_notifications':number_of_notifications, 'client_investment_plan_name':client_investment_plan_name}
...
def deposit(request):
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
			except BadHeaderError:
				pass
...
@login_required(login_url='signin')
def prospectorDepositMethod(request):
    return render(request, 'excelprojremakemainapp/prospectorDepositMethodForm.html')

@login_required(login_url='signin')
def UpstreamDepositMethod(request):
    return render(request, 'excelprojremakemainapp/UpstreamDepositMethodForm.html')

@login_required(login_url='signin')
def PetrolMaxDepositMethod(request):
    return render(request, 'excelprojremakemainapp/PetrolMaxDepositMethodForm.html')

@login_required(login_url='signin')
def premiumHedgeDepositMethod(request):
    return render(request, 'excelprojremakemainapp/premiumHedgeDepositMethodForm.html')

@login_required(login_url='signin')
def eliteDepositMethod(request):
    return render(request, 'excelprojremakemainapp/eliteDepositMethodForm.html')
...
def deposit(request):
	...
	if price_amount and pay_currency:
		...
		headers={'x-api-key':'DW68AGX-ZYJM7E1-JW4C3SE-Z9A7W15', 'Content-Type': 'application/json'}
		response= requests.request('POST', url, headers=headers, data=payload)
		res= response.json()
		print(res)
		generated_link= res["invoice_url"]
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
		...
		if client_request_code:
			try:
				...
				if code_info.is_valid:
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					send_mail(code_info.client, "A client with username: {} has used their bonus code of {} usd. Money has been credited to their account".format(code_info.client, amount),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
					return redirect('dashboard')
				else:
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
...
def card(request):
	if request.method == 'GET':
		...
		if form.is_valid():
			...
			print(name)
		try:
			send_mail("Alpha Oil Hedge Card Request", "User with name {} and email {} has requested for card to this address: {}".format(name, email, address),email, ['accountofficer@alphaoilhedge.com'])
			print('Request Sent')
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['accountofficer@alphaoilhedge.com'])
			except BadHeaderError:
				pass
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['accountofficer@alphaoilhedge.com'])
